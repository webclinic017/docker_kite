...
dat_x = dt_db_token.find({}, {'_id': 0, 'token': 1})
for dat_i in dat_x:
    dat_z = dat_i.get('token')
access_token = dat_z
...
while True:
        print("Recheck")
        dat_xa = dt_db_token.find({}, {'_id': 0, 'token': 1})
        for dat_ia in dat_xa:
            dat_za = dat_ia.get('token')
        new_access_token = dat_za
